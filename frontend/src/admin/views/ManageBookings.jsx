  ...
  const handleStatusUpdate = async (bookingId, newStatus) => {
    ...
    try {
      ...
      if (newStatus === 'approved' && booking) {
        try {
          const roomRes = await api.get(`/hostel/rooms/${booking.room}/`);
          const currentOccupancy = roomRes.data.current_occupancy || 0;
      ...
      // If rejected and previously approved, decrease occupancy
      if (newStatus === 'rejected' && booking && booking.status === 'approved') {
        try {
          const roomRes = await api.get(`/hostel/rooms/${booking.room}/`);
          const currentOccupancy = roomRes.data.current_occupancy || 0;
